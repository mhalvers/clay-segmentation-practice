...
class ChesapeakeSegmentor(L.LightningModule):
    ...
    def __init__(  # # noqa: PLR0913
        ...
        self.loss_fn = FocalLoss(gamma=2.0, alpha=1.0)
    ...
    def forward(self, datacube):
        ...
        pixels = datacube["pixels"].to(device=device, dtype=dtype)
        time = datacube["time"].to(device=device, dtype=dtype)
        latlon = datacube["latlon"].to(device=device, dtype=dtype)
        waves = torch.tensor([0.65, 0.56, 0.48, 0.842], device=device, dtype=dtype)
        gsd = torch.tensor(1.0, device=device, dtype=dtype)  # NAIP GSD
    ...
    def shared_step(self, batch, batch_idx, phase):
        ...
        """
        outputs = self(batch)
        labels = batch["label"].long().to(outputs.device)
